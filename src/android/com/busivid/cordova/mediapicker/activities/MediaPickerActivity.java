...
import com.busivid.cordova.mediapicker.CropListener;
...
import com.busivid.cordova.mediapicker.utils.MessageUtils;
import com.busivid.cordova.mediapicker.utils.RecursiveFileObserver;
...
public class MediaPickerActivity extends AppCompatActivity implements MediaSelectedListener, CropListener, FragmentManager.OnBackStackChangedListener, FragmentHost {
			...
			if (max != Integer.MAX_VALUE) {
				...
				if (mMediaOptions.isShowWarningVideoDuration()) {
					MediaPickerErrorDialog dialog = MediaPickerErrorDialog
							.newInstance(MessageUtils.getWarningMessageVideoDuration(getApplicationContext(), max));
	...
	private void tryCorrectPhotoFileCaptured() {
		...
		for (File file : mFilesCreatedWhileCapturePhoto) {
			if (MediaUtils.isImageExtension(MediaUtils.getFileExtension(file)) && file.length() >= captureSize
	...
	private class FileObserverTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			if (mFileObserver == null) {
				mFileObserver = new RecursiveFileObserver(Environment.getExternalStorageDirectory().getAbsolutePath(),
						FileObserver.CREATE);
